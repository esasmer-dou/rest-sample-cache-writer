...
[Kısa Kullanıcı Rehberi](docs/USER_GUIDE.tr.md) | [PDF](docs/rest-sample-cache-writer-user-guide.tr.pdf)
...
Bu örnek `java-rust-cache` ve `rust-sample-model` bağımlılıklarını GitHub Packages üzerinden çeker. Maven bu paketleri indirebilmek için kimlik bilgisi ister; bu GitHub Packages'in normal erişim modelidir.
...
| `reactor.cache.redis.access-mode` | `write-only` | Yalnız native write plane'i açar. Read çağrısını JNI'ye ulaşmadan reddeder. | Bu scheduled writer'da `write-only` bırak. Process cache verisi de okuyacaksa `read-write` kullan. |
